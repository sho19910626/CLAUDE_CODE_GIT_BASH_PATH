...
import { VERDICT_CLASS, VERDICT_LABEL, num, pct } from "./format";
...
export default function Dashboard({
  ...
  const rows = useMemo(() => {
    const list = analysis.jobs.filter((j) => showArchived || !j.job.archived);
    const gainOf = (gains: number[]) => Math.max(0, ...gains, 0);
    return [...list].sort((a, b) => {
      switch (sort) {
        ...
        default:
          return (
            gainOf(b.diagnosis.stages.map((s) => s.potentialGain)) -
            gainOf(a.diagnosis.stages.map((s) => s.potentialGain))
          );
      }
    });
  }, [analysis, sort, showArchived]);
    ...
    <>
      <div className="idd-kpis">
        ...
        <Kpi
          ...
          sub="各求人の最大ボトルネックを上位25%水準に戻した場合"
          accent
        />
      ...
      {rows.length === 0 ? (
        ...
        <div className="panel idd-table-wrap">
          <table className="idd-table">
            ...
            <tbody>
              {rows.map(({ job, diagnosis, recommendations }) => {
                ...
                const best = [...diagnosis.stages].sort(
                ...
                return (
                  <tr key={job.id} onClick={() => onSelect(job.id)}>
                    ...
                    <td>
                      {top ? (
                        <>
                          <div className="idd-toprec">{top.title}</div>
                          <div className="idd-sub">
                            {VERDICT_LABEL[best.verdict]}・{best.label.split("(")[0]}
                          </div>
                        </>
                      ) : (
                        <span className="idd-sub">
                          {diagnosis.bottleneck === null
                            ? "明確な課題なし"
                            : "判定に必要なデータが不足"}
                        </span>
                      )}
                    ...
                    <td className="rt">
                      <span className="idd-gain">+{best.potentialGain.toFixed(1)}</span>
          ...
          <p className="idd-note">
            ...
            。「伸びしろ」は各求人で最も詰まっている段階を、同セグメント上位25%の水準まで戻した場合の月間応募増の試算です。
          </p>
        </div>
      )}
    </>
