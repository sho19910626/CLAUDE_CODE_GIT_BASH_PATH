...
// ようにするため。署名だけ見ていると、止めた後もセッションが切れるまで
// 営業リスト(/console.html)などを開けてしまう。
//
// 毎回問い合わせると重いので、同じ実行インスタンスの中では 30 秒だけ結果を覚える。
...
const MAX_AGE_MS = 60 * 60 * 12 * 1000; // 12時間。auth.ts と揃える
...
async function isAccountActive(userId: string): Promise<boolean> {
  ...
  try {
    ...
    const rows = (await sql`
      select active from idd_users where id = ${userId}
    `) as { active: boolean }[];
