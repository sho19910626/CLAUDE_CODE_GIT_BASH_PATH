...
const IMPRESSION_ACTIONS: PlaybookAction[] = [
  ...
  },
  {
    ...
    stage: "impressions",
    title: "求人タイトルに勤務地(市区町村)を入れる",
    why:
      "求職者の検索は「職種 + 地名」が大半です。都道府県だけでなく市区町村まで入っていると、通勤圏で探している層の検索に直接ヒットします。",
    how: [
      "「【○○市】フォークリフト/日勤のみ」のように、タイトル先頭に市区町村を置く",
      "最寄駅・バス停も本文に書く(「○○駅から徒歩 8 分」)",
      "複数拠点をまとめている場合は拠点ごとに求人を分ける",
    ],
    seedLift: 0.15,
    effort: 1,
    when: (c) => {
...
const CTR_ACTIONS: PlaybookAction[] = [
  ...
  },
  {
    id: "ctr-title-hook",
    stage: "ctr",
    title: "求人タイトルに、一覧で目を引く条件を 1 つ足す",
    why:
      "タイトルは一覧で最も大きく表示されます。「未経験可」「土日休み」「日払いOK」など、その求人で本当に言える条件が 1 つ入るだけでクリック率が変わります。",
    how: [
      "職種名 + 勤務地 + 条件 1 つ、の順で 30 文字前後に収める",
