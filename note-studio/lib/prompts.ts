...
写真を自分で用意する場合の指示。何を写すか、どう撮るか。
写真が無くても記事は出せるので、ここは「あとで差し替えるなら」の参考として書く。

## coverImage — そのまま画像になる見出し画像の設計

このツールは、ここに書かれた内容から**見出し画像を実際に描いて出力します**。
写真が無くても、これだけで note に載せられる画像になります。

- headline: 画像に大きく載せる文字。**12〜26 文字**。
  **タイトルの丸写しにしない。** タイトルは検索向け、画像の文字は
  一覧でスクロールする指を止めるためのもの。役割が違う。
  記事の核心を、短く言い切る。体言止めや短い問いが効く。
- sub: 小さく添える一行。**20 文字以内**。無理に付けず、要らなければ空文字。
- layout: 記事の中身で選ぶ。
    band   … 左に縦帯。汎用。迷ったらこれ
    center … 中央寄せ。宣言・まとめ記事向け
    quote  … 引用符つき。セリフ・体験談を見出しにするとき
- bg: 背景色。「#rrggbb」の形式(例 #0f1720)。
  **落ち着いた濃い色を選ぶ**(#0f1720 / #1d2b3a / #2b1f2e など)。
  彩度の高い原色は安っぽく見えるので避ける。
  記事の内容に合わせて、シリーズで揃うトーンにする。
- accent: 差し色。同じく「#rrggbb」。背景と明確に差があること。帯や罫線に使う。
- fontStyle: gothic(標準・力強い) / mincho(落ち着き・内省的) / rounded(やわらかい)

**文字色は指定しません。** 背景色から読みやすいほうを自動で選びます。
そのため bg は「白文字が載る濃さ」か「黒文字が載る明るさ」か、
どちらかにはっきり寄せてください。中途半端な明度は避けること。
